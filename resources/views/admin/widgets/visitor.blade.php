<div class="flex items-end justify-between gap-2">
    ...
    <div class="h-10 w-24">
        <svg viewBox="0 0 100 40" class="w-full h-full" preserveAspectRatio="none">
            <defs>
                <linearGradient id="v-{{ \Illuminate\Support\Str::slug($widget->label) }}" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stop-color="{{ $widget->up ? '#6366f1' : '#ef4444' }}" stop-opacity="0.25" />
                    <stop offset="100%" stop-color="{{ $widget->up ? '#6366f1' : '#ef4444' }}" stop-opacity="0" />
                </linearGradient>
            </defs>
            @php
                $pts = $widget->data;
            ...
            @endphp
            <path d="M0,40 L{{ $points }} L100,40 Z" fill="url(#v-{{ \Illuminate\Support\Str::slug($widget->label) }})" />
            <polyline fill="none" stroke="{{ $widget->up ? '#6366f1' : '#ef4444' }}" stroke-width="1.75" points="{{ $points }}" />
        </svg>
    </div>
</div>
