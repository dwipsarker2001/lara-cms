    ...
    <div class="flex items-end justify-between gap-2">
        ...
        <div class="h-10 w-24 shrink-0">
            <svg viewBox="0 0 100 40" class="w-full h-full" preserveAspectRatio="none">
                <defs>
                    <linearGradient id="g-form-stat-{{ $selectedFormId ?? 'all' }}" x1="0" y1="0" x2="0" y2="1">
                ...
                @php
                    ...
                    $w = 100;
                    $h = 40;
                    $step = count($pts) > 1 ? $w / (count($pts) - 1) : 0;
                    $points = implode(' ', array_map(fn($i, $v) => round($step * $i, 1).','.round($h - (($v - $min) / $range) * $h, 1), array_keys($pts), $pts));
                @endphp
                <path d="M0,40 L{{ $points }} L100,40 Z" fill="url(#g-form-stat-{{ $selectedFormId ?? 'all' }})" />
