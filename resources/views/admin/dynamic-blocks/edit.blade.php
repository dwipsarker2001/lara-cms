...
<div class="max-w-5xl mx-auto px-2 sm:px-0">
    ...
    <form id="block-form" method="POST" action="{{ route('admin.dynamic-blocks.update', $block) }}">
        @csrf @method('PUT')
                        ...
                        <div class="divide-y divide-content-border">
                            <div class="grid md:grid-cols-2 items-start px-[18px] py-4 gap-y-3 md:gap-y-0 md:gap-x-5">
                                <div class="flex flex-col gap-1.5">
                                    <span class="text-sm font-medium text-text-heading">ID</span>
                                </div>
                                <div class="text-sm text-text-muted py-1.5">{{ $block->name }}</div>
                            </div>
                            <div class="grid md:grid-cols-2 items-start px-[18px] py-4 gap-y-3 md:gap-y-0 md:gap-x-5">
                                <div class="flex flex-col gap-1.5">
                                    <label for="field-label" class="text-sm font-medium text-text-heading">Label</label>
                                    <div class="text-sm text-text-muted">This name will appear in the block picker when adding a block to your page.</div>
                                </div>
                                <div>
                                    <input id="field-label" type="text" name="label" value="{{ old('label', $block->label) }}" class="w-full block bg-content-bg border border-content-border text-text-primary placeholder:text-text-muted text-sm rounded-lg px-3 py-2 h-9 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                                </div>
                            </div>
                            <div class="grid md:grid-cols-2 items-start px-[18px] py-4 gap-y-3 md:gap-y-0 md:gap-x-5">
                                <div class="flex flex-col gap-1.5">
                                ...
                                <x-admin::toggle-switch model="global" name="global" :value="old('global', $block->global) ? 'true' : 'false'" />
                            </div>
                        </div>
                    </div>
                </div>
