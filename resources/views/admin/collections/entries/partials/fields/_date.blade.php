...
<template x-if="field.type === 'date' || field.type === 'datetime' || field.type === 'datetime-local'">
    <div x-data="{ showSourcePicker: false }">
        ...
        <template x-if="isSourceField(field.name)">
            <input :type="(field.enable_time || field.type === 'datetime' || field.type === 'datetime-local') ? 'datetime-local' : 'date'"
                :value="getField(field.name)" disabled
        ...
        <template x-if="!isSourceField(field.name)">
            <input :type="(field.enable_time || field.type === 'datetime' || field.type === 'datetime-local') ? 'datetime-local' : 'date'"
                :value="getField(field.name)" @input="setField(field.name, $event.target.value)"
