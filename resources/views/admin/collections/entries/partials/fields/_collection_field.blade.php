{{-- collection field / key mapper matching select input design --}}
<template x-if="field.type === 'collectionField' || field.type === 'collection_field' || field.type === 'collectionKey'">
    <div>
        ...
        <div class="relative" x-data="{ open: false }">
            ...
            >
                <span class="flex-1 text-left truncate" x-text="getCollectionFieldOptions(field).find(o => o.value === getField(field.name))?.label || (getField(field.name) || 'Select...')"></span>
                <svg class="size-4 text-text-muted transition-transform shrink-0" :class="open ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
            </button>
            ...
            >
                <template x-for="opt in getCollectionFieldOptions(field)" :key="opt.value">
                ...
                <template x-if="getCollectionFieldOptions(field).length === 0">
                    <div class="px-3 py-2 text-sm text-gray-400">Select a Collection first to pick fields</div>
                </template>
            </div>
        </div>
    </div>
