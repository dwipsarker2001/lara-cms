    ...
    <form id="entry-form" method="POST" action="{{ route('admin.collections.entries.update', [$collection, $entry]) }}">
        ...
        <div class="bg-panel-bg rounded-2xl mb-8 p-[7px]">
            <div class="px-[18px] pt-3 pb-1">
                ...
                <p class="text-sm text-text-muted mt-1">Update the fields for this entry.</p>
            </div>
                    ...
                    <div class="divide-y divide-content-border">
                        <div class="grid md:grid-cols-2 items-start px-[18px] py-4 gap-y-3 md:gap-y-0 md:gap-x-5">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="flex-1">
                                    <input id="field-title" type="text" name="data[title]" value="{{ old('data.title', $entry->data['title'] ?? '') }}" placeholder="Enter title" class="w-full block bg-content-bg border border-content-border text-text-primary placeholder:text-text-muted text-sm rounded-lg px-3 py-2 h-9 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                                </div>
                            </div>
                        </div>
                        @forelse($collection->fields ?? [] as $field)
                            @php
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="flex-1">
                                        @switch($field['type'] ?? 'text')
                                            @case('textarea')
                                                <textarea id="field-{{ $loop->index }}" name="data[{{ $key }}]" rows="3" class="w-full block bg-content-bg border border-content-border text-text-primary placeholder:text-text-muted text-sm rounded-lg px-3 py-2 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">{{ old('data.' . $key, $value) }}</textarea>
                                                @break
                                            @case('number')
                                                <input type="number" id="field-{{ $loop->index }}" name="data[{{ $key }}]" value="{{ old('data.' . $key, $value) }}" class="w-full block bg-content-bg border border-content-border text-text-primary placeholder:text-text-muted text-sm rounded-lg px-3 py-2 h-9 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                                                @break
                                            @default
                                                <input type="text" id="field-{{ $loop->index }}" name="data[{{ $key }}]" value="{{ old('data.' . $key, $value) }}" placeholder="{{ $field['title'] ?? '' }}" class="w-full block bg-content-bg border border-content-border text-text-primary placeholder:text-text-muted text-sm rounded-lg px-3 py-2 h-9 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary">
                                        @endswitch
                                    </div>
                                </div>
                            </div>
                        @empty
                            <div class="px-[18px] py-8 text-center text-sm text-text-muted">
                                No additional fields defined. You can <a href="{{ route('admin.collections.edit', $collection) }}" class="text-primary hover:text-primary/80 no-underline font-medium">edit this collection</a> to add custom fields.
                            </div>
                        @endforelse
                    </div>
                </div>
