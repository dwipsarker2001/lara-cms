...
<div data-block="login" class="min-h-screen w-full flex bg-white">
    <div class="w-full lg:w-1/2 flex items-center justify-center px-6 py-16">
        <div class="w-full max-w-sm">
            ...
            <h1 data-edit="headline" class="text-center text-[26px] leading-tight font-bold text-gray-900 mb-8">
                {{ $d['headline'] ?? 'Bring your product stories to life in minutes' }}
            </h1>
            @if($d['subtitle'] ?? false)
            ...
            <div class="space-y-3">
                <a href="{{ url('/auth/google') }}"
                   class="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl px-4 py-3 text-sm font-semibold text-gray-800 hover:bg-gray-50 transition shadow-sm no-underline">
                    ...
                    <span data-edit="googleLabel">{{ $d['googleLabel'] ?? 'Continue with Google' }}</span>
                </a>

                @if($d['microsoftEnabled'] ?? true)
                <button type="button" class="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl px-4 py-3 text-sm font-semibold text-gray-800 hover:bg-gray-50 transition shadow-sm">
                    ...
                    <span data-edit="microsoftLabel">{{ $d['microsoftLabel'] ?? 'Continue with Microsoft' }}</span>
                </button>
                @endif
                ...
                <button type="button" class="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl px-4 py-3 text-sm font-semibold text-gray-800 hover:bg-gray-50 transition shadow-sm">
                    ...
                    <span data-edit="appleLabel">{{ $d['appleLabel'] ?? 'Continue with Apple' }}</span>
                </button>
                @endif
            </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-3">
                @csrf
                <input type="email" name="email" value="{{ old('email') }}" placeholder="{{ $d['emailPlaceholder'] ?? 'Enter your email...' }}" required autofocus autocomplete="username"
                ...
                <input data-edit="passwordPlaceholder" type="password" name="password" placeholder="{{ $d['passwordPlaceholder'] ?? 'Password' }}" required autocomplete="current-password"
                ...
                <div class="flex items-center justify-between">
                    <label class="flex items-center gap-2 text-sm text-gray-500 cursor-pointer">
                        <input type="checkbox" name="remember" class="size-4 rounded border-gray-300 accent-gray-900">
                        <span data-edit="rememberLabel">{{ $d['rememberLabel'] ?? 'Remember me' }}</span>
                    </label>
                </div>
                <button type="submit" data-edit="continueLabel" data-edit-button class="w-full rounded-xl bg-gray-900 px-4 py-3 text-sm font-semibold text-white hover:bg-gray-800 transition shadow-sm">
                    {{ $d['continueLabel'] ?? 'Continue' }}
                </button>
            </form>

            @if(($d['showRegisterLink'] ?? true) && ($d['registerLabel'] ?? false))
            <p class="text-center text-sm text-gray-400 mt-6">
                <span data-edit="registerLabel">{{ $d['registerLabel'] }}</span>
                <a href="{{ $d['registerUrl'] ?? url('/register') }}" data-edit="registerLinkText" class="font-semibold text-gray-900 hover:underline">{{ $d['registerLinkText'] ?? 'Create one' }}</a>
            </p>
            @endif
        </div>
    </div>
